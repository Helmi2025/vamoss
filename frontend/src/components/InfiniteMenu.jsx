...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    // Fill with a neutral placeholder color first so the sphere never renders fully transparent/black
    ...
    const loadImage = src =>
      new Promise(resolve => {
        ...
        img.onerror = () => resolve(null); // fail gracefully, keep placeholder cell
        img.src = src;
      });

    Promise.all(this.items.map(item => loadImage(item.image))).then(images => {
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div className="infinite-menu-wrapper">
      <canvas className="infinite-grid-menu-canvas" ref={canvasRef} />
