...
function AvatarSection({ userId, photoUrl, onPhotoChange }) {
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      const res = await fetch(`/api/captain/profile/${userId}/photo`, {
        method: 'POST',
...
function CaptainManageProfile() {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem('token')
    fetch(`/api/captain/profile/${user.userId}/photo`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(r => r.json())
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true)
      const token = localStorage.getItem('token')
      const res = await fetch(`/api/captain/profile/${user.userId}`, {
        method: 'PUT',
        headers: {
...
export default CaptainManageProfile
