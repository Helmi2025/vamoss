...
/* ══════════════════════════════════════════════════════════
   CONFIRM MODAL
══════════════════════════════════════════════════════════ */
function ConfirmModal({ action, captain, reason, onReasonChange, loading, onConfirm, onClose }) {
      ...
      <div className="modal-box sm">
        <div className="modal-header">
          <h2 className="modal-title">
            {isApprove ? 'Approve Captain' : 'Reject Application'}
          </h2>
        ...
        <p className="confirm-text">
          {isApprove ? (
            <>Are you sure you want to approve <span className="confirm-name">{captain.fullName}</span>? They will receive an email and gain access to the platform.</>
          ) : (
            <>Are you sure you want to reject <span className="confirm-name">{captain.fullName}</span>? They will be notified by email.</>
          )}
        </p>
...
/* ══════════════════════════════════════════════════════════
   MAIN COMPONENT
══════════════════════════════════════════════════════════ */
function CaptainApplications({ onCountChange }) {
  ...
  const fetchApplications = async () => {
    ...
    setError('')
    try {
      const { data } = await api.get('/api/admin/captain-applications')
  ...
  /* ── Modal helpers ── */
  const openConfirm = (action, captain) => {
    setReason('')
    setConfirm({ action, captain })
  }
  ...
  const handleAction = async () => {
    const { action, captain } = confirm
    setActLoading(true)
    try {
      if (action === 'approve') {
        await api.put(`/api/admin/captain-applications/${captain.userId}/approve`)
        pushToast('success', `${captain.fullName} has been approved. A confirmation email was sent.`)
      } else {
        await api.put(`/api/admin/captain-applications/${captain.userId}/reject`, {
          reason: reason.trim() || null,
        })
        pushToast('success', `${captain.fullName}'s application has been rejected.`)
      }
      setApplications((prev) => {
        const updated = prev.filter((a) => a.userId !== captain.userId)
  ...
  return (
    <div>
      <div className="page-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 className="page-title">Captain Applications</h1>
      ...
      <div className="content-card" style={{ padding: 0 }}>
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>No pending captain applications.</p>
          </div>
        ) : (
          <div className="app-table-wrap">
            <table className="app-table">
              <thead>
                <tr>
                  <th>Captain</th><th>Email</th><th>Team</th>
                  <th>Sport</th><th>Applied</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
...
export default CaptainApplications
