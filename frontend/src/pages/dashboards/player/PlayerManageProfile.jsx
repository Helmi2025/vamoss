...
function AvatarSection({ userId, photoUrl, onPhotoChange }) {
  ...
  const clearFeedback = () => { setPhotoError(''); setPhotoSuccess('') }
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      const res = await fetch(`/api/player/profile/${userId}/photo`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      })
      const data = await res.json()
      if (!res.ok) { setPhotoError(data.message || 'Failed to upload photo.'); return }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 28 }}>
      <div style={{ position: 'relative', width: 140, height: 140 }}>
        ...
        <div
          ...
          style={{
            ...
            background: dragOver ? 'rgba(198,168,75,0.18)' : photoUrl ? 'transparent' : 'rgba(198,168,75,0.08)',
        ...
        {photoUrl && !photoLoading && (
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); clearFeedback(); setShowDeleteModal(true) }}
            title="Remove photo"
            style={{
      ...
      {photoError   && <p style={{ color: '#e57373', fontSize: 12, marginTop: 4, textAlign: 'center' }}>{photoError}</p>}
      {photoSuccess && <p style={{ color: '#81c784', fontSize: 12, marginTop: 4, textAlign: 'center' }}>{photoSuccess}</p>}

      {showDeleteModal && (
        <DeletePhotoModal onConfirm={handleDelete} onClose={() => setShowDeleteModal(false)} />
      )}
    </div>
  )
}
...
function PlayerManageProfile() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/player/profile/${user.userId}/photo`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(r => r.json())
    ...
    fetch(`/api/player/profile/${user.userId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(r => r.json())
      .then(data => {
        if (data.gender)    setGender(data.gender)
        if (data.sportName) setSportName(data.sportName)
      })
      .catch(() => {})
  }, [user?.userId])
  ...
  const handleSave = async () => {
    ...
    if (!currentPassword) { setError('Current password is required to save any changes.'); return }
    ...
    if (newPassword) {
      ...
      if (!/[a-z]/.test(newPassword))      { setError('New password must contain at least one lowercase letter.'); return }
    ...
    try {
      setLoading(true)
      const token = localStorage.getItem('token')
      const res = await fetch(`/api/player/profile/${user.userId}`, {
        method: 'PUT',
        headers: {
        ...
        <div className="content-card" style={{ maxWidth: 860, width: '100%', position: 'relative' }}>
          ...
          <button
            ...
            style={{
              ...
              color: '#c6a84b', transition: 'background 0.2s',
            }}
          >
          ...
          <AvatarSection
            userId={user?.userId}
            photoUrl={photoUrl}
            onPhotoChange={url => setPhotoUrl(url || null)}
          />

          {/* Read-only info: Gender + Sport */}
          {(gender || sportName) && (
            <div style={{ display: 'flex', gap: 16, marginBottom: 20, justifyContent: 'center', flexWrap: 'wrap' }}>
              {gender && (
                <div style={{
                  ...
                  <span style={{ fontSize: 11, color: 'rgba(198,168,75,0.6)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Gender</span>
                  <span style={{
                    fontSize: 14, fontWeight: 600,
                    color: gender === 'MALE' ? '#7eb8f7' : '#f7a8c4',
                  }}>
                    {gender === 'MALE' ? '♂ Man' : '♀ Woman'}
                  </span>
                  <span style={{ fontSize: 10, color: 'rgba(198,168,75,0.35)', fontStyle: 'italic' }}>(fixed)</span>
                </div>
              )}
              {sportName && (
                <div style={{
                  ...
                  <span style={{ fontSize: 11, color: 'rgba(198,168,75,0.6)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Sport</span>
                  <span style={{ fontSize: 14, fontWeight: 600, color: '#c6a84b' }}>{sportName}</span>
                  <span style={{ fontSize: 10, color: 'rgba(198,168,75,0.35)', fontStyle: 'italic' }}>(fixed)</span>
                </div>
              )}
            </div>
          ...
          {/* Row 2: Email + New Password + Confirm */}
          ...
          {/* Feedback */}
          {error   && <p style={{ color: '#e57373', fontSize: 13, marginTop: 16, textAlign: 'center' }}>{error}</p>}
          {success && <p style={{ color: '#81c784', fontSize: 13, marginTop: 16, textAlign: 'center' }}>{success}</p>}
...
export default PlayerManageProfile
